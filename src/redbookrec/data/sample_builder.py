...
def _positive_rows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for base in df.itertuples(index=False):
        ...
        for item in details:
            ...
            if int(float(detail.get("click", 0) or 0)) <= 0:
                continue
            try:
                pos_note_idx = int(detail.get("note_idx"))
            except Exception:
                continue
            rows.append(
                {
...
def _extend_note_map(map_path: Path, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    values: list[int] = []
    for row in rows:
        values.append(int(row["pos_note_idx"]))
...
def build_recall_samples(cfg: dict, max_requests: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    train_rows = _positive_rows(train_df)
    test_rows = _positive_rows(test_df)
