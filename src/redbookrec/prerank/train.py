...
from redbookrec.prerank.dataset import PrerankDataset, build_prerank_train_frame, build_user_map, dense_dim
from redbookrec.prerank.dcn import DCN
from redbookrec.prerank.loss import build_pos_weight, click_bce_loss
from redbookrec.recall.dataset import NOTE_DENSE_COLS, USER_DENSE_COLS, build_note_features, build_user_features
    ...
    torch.save(
        {
            ...
            "hidden_dims": list(cfg["model"].get("hidden_dims", [128, 64])),
...
def train_prerank(cfg: dict, smoke_test: bool = False) -> dict:
    ...
    samples = build_prerank_train_frame(rec_df)
    ...
    if samples.empty:
        raise RuntimeError("no prerank samples found")

    note_map = read_json(cfg.get("data", {}).get("note_id_map_path", "data_cache/notes/note_id_map.json"))["raw_to_model"]
    ...
    if samples.empty:
        raise RuntimeError("no prerank samples can be mapped to note ids")
    ...
    dataset = PrerankDataset(samples, note_map, user_map, note_features, user_features)
    loader = DataLoader(
        dataset,
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=float(cfg["train"].get("lr", 0.001)), weight_decay=float(cfg["train"].get("weight_decay", 0.0)))
    ...
    for epoch in range(int(cfg["train"].get("epochs", 1))):
        running = 0.0
        seen = 0
        pbar = tqdm(loader, desc=f"train_dcn epoch={epoch + 1}", leave=False)
        for batch in pbar:
            ...
            logits = model(user_id, note_id, user_cat, note_type, note_tax, dense)
